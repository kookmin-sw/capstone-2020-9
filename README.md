...
3)OpenCV와 FFmpeg를 설치한다.
    ...
    opt2) <span style="color:red">setup_opencv.sh</span>를 실행하여 OpenCV를 자동 빌드한다. 그리고 Mediapipe의 OpenCV 구성을 수정한다.
    opt3) https://docs.opencv.org/3.4.6/d7/d9f/tutorial_linux_install.html (OpenCV manual)을 따라한다.
    
    추가. Mediapipe가 당신 컴퓨터의 OpenCV의 라이브러리를 가리키도록 <span style="color:red">WORKSPACE</span> 와 opencv_linux.BUILD 를 수정해야한다. 
    만약 OpenCV4가 "/usr/local"에 설치되어있다면 당신은 WORKSPACE 의 "linux_opencv"new_local_repository rule과 opencv_linux.BUILD 의 "opencv" cc_library ruls을 수정해야한다.
1) server
    `running on aws`  
    `ip : 35.175.201.165`  
    `port : 8081`  
