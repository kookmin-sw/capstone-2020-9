...
extern "C" {
            ...
            (
                ...
                for( int i=0; i<lines.size(); i++ )// 위치에 따른 직선 분류
                {
                    ...
                    else if (abs(L[3]-L[1]) > 4*abs(L[2]-L[0])){ // 세로선, 기울기가 4보다 큰 경우
                        if(L[0]<width/3){
                            line3.push_back(L);
                        }
                        else if (L[0]>width*2/3){
                            line4.push_back(L);
                        }
                    }
                }

                double SigA, SigB; // SigA 기울기의 합, SigB는 y절편의 합
                double a1, b1;
                double a2, b2;
                double a3, b3;
                double a4, b4;
                int c3=-1;// x=N 형태의 직선 나타내기 위해
                int c4=-1;
